...
account_balances_max <- trans_test %>% group_by(account_id) %>% slice(which.max(balance))
account_balances_max <- subset(account_balances_max, select = c("trans_id","balance"))
account_balances_min <- trans_test %>% group_by(account_id) %>% slice(which.min(balance))
account_balances_min <- subset(account_balances_min, select = c("trans_id","balance"))

account_household <- trans_test %>% slice(which(k_symbol == 1))
...
loan_test <- merge(loan_test, sd_balance, by = "account_id")
loan_test <- subset(loan_test, select = -amount)

loans_household <- left_join(loan_test, account_household)
