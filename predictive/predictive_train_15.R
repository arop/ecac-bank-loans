...
disp_owners <- disp %>% filter(type == "OWNER")
...
#household
account_household_train <- trans_train %>% slice(which(k_symbol == "household"))
...
account_household_test <- trans_test %>% slice(which(k_symbol == "household"))
...
#join cards
card_account_train <- merge(card_train, disp, by.x="disp_id", by.y="disp_id", all.x = TRUE)
card_account_test <- merge(card_test, disp, by.x="disp_id", by.y="disp_id", all.x = TRUE)

card_account_train <- subset(card_account_train, select = -c(`type.y`, disp_id, card_id, client_id, issued))
card_account_test <- subset(card_account_test, select = -c(`type.y`, disp_id, card_id, client_id, issued))

loan_train <- merge(loan_train, card_account_train, all.x = TRUE)
loan_test <- merge(loan_test, card_account_test, all.x = TRUE)

colnames(loan_train)[28] <- "type_card"
colnames(loan_test)[28] <- "type_card"

loan_train$type_card[is.na(loan_train$type_card)] <- "none"
loan_test$type_card[is.na(loan_test$type_card)] <- "none"
...
write.csv(loan_train, file = "C:\\Repositories\\ecac-bank-loans\\datasets\\train_15.csv", row.names = FALSE)
write.csv(loan_test, file = "C:\\Repositories\\ecac-bank-loans\\datasets\\test_15.csv", row.names = FALSE)